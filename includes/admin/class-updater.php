...
class Updater {
    ...
    public function __construct($user, $repo, $branch = 'main') {
        $this->user = $user;
        $this->repo = $repo;
        $this->branch = $branch;
        $this->plugin_slug = 'meowfield/meowfield.php';
        $this->plugin_file = MEOWFIELD_PLUGIN_FILE;
    ...
    public function add_action_links($links) {
        $check_url = wp_nonce_url(admin_url('plugins.php?meowfield_check_update=1'), 'meowfield_check_update');
        $links[] = '<a href="' . esc_url($check_url) . '">' . __('Check for Updates', 'meowfield') . '</a>';
        return $links;
    }

    public function manual_update_check() {
        if (isset($_GET['meowfield_check_update']) && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'meowfield_check_update')) {
            delete_transient('meowfield_latest_commit_check');
    ...
    private function get_latest_commit() {
        $transient_key = 'meowfield_latest_commit_check';
    ...
    public function check_update($transient) {
        ...
        $current_commit = get_option('meowfield_current_commit', '');
    ...
    public function plugin_info($false, $action, $response) {
        if ($action !== 'plugin_information' || empty($response->slug) || $response->slug !== 'meowfield') {
            return $false;
        }
    ...
    public function rename_github_extracted_folder($source, $remote_source, $upgrader, $hook_extra = null) {
        ...
        if (isset($hook_extra['plugin']) && $hook_extra['plugin'] === $this->plugin_slug) {
            // Because github branches become e.g. meowfield-main we need to change it
            $extracted_dir = basename($source);
            $expected_dir = 'meowfield';
    ...
    public function update_completed($upgrader_object, $options) {
        if ($options['action'] === 'update' && $options['type'] === 'plugin' && !empty($options['plugins'])) {
            if (in_array($this->plugin_slug, $options['plugins'])) {
                $latest_commit = $this->get_latest_commit();
                if ($latest_commit) {
                    update_option('meowfield_current_commit', $latest_commit['sha']);
                }
            }
        }
    }
